...
void setup(void)
{
  ...
  myFile.println("dt (ms),time (sec),acc_x_JY (m/s^2),acc_y_JY (m/s^2),acc_z_JY (m/s^2),acc_x_corr_JY (m/s^2),acc_y_corr_JY (m/s^2),roll_JY (deg),pitch_JY (deg),yaw_JY (deg),acc_x_BNO (m/s^2),acc_y_BNO (m/s^2),acc_z_BNO (m/s^2),acc_x_corr_BNO (m/s^2),acc_y_corr_BNO (m/s^2),roll_BNO (deg),pitch_BNO (deg)");
}
...
void loop(void)
{
  ...
  if (recording == 1){
    //broke the print statement into 2 because it had too many terms in the print statement
    myFile.print((String) dt+","+t_1/1000+",");
    myFile.print((String) acc_x_JY+","+acc_y_JY+","+acc_z_JY+","+acc_x_corr_JY+","+acc_y_corr_JY+","+roll_JY+","+pitch_JY+","+yaw_JY+",");
    myFile.println((String) acc_x_BNO+","+acc_y_BNO+","+acc_z_BNO+","+acc_x_corr_BNO+","+acc_y_corr_BNO+","+roll_BNO+","+pitch_BNO);
  }
